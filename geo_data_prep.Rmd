...
```
## Step 1: Get all Series records
### Series record 1: Fetch study GSE147507 from GEO with 7 comparisons 
```{r series_record_1}
# Fetch count matrix 
...
### Series record 2: Fetch study GSE151879 from GEO with 3 comparisons
```{r series_record_2}
...
### Series record 3: Fetch study GSE148696 from GEO with 1 comparisons
```{r series_record_3}
  getGEOSuppFiles("GSE148696")
# Comp. 3.11: Samples with SARS-CoV-2 infected and mock treated HPSC-derived Colonic organoids. GSE148696 ####
...
### Series record 4: Fetch study GSE164073 from GEO with 3 comparisons
```{r series_record_4}
...
# Comp. 4.12: Cornea Samples SARS-CoV-2 infected and mock treated ####
...
# Comp. 4.13: Limbus Samples SARS-CoV-2 infected and mock treated ####
...
# Comp. 4.14: Sclera Samples SARS-CoV-2 infected and mock treated ####
...
### Series record 5: Fetch study GSE160435 from GEO with 1 comparison 
```{r series_record_5}
  ...
  # Comp. 5.15:  Samples with SARS-CoV-2 infected and mock treated AT2 cells####
...
### Series record 6: Fetch study GSE157852 from GEO with 2 comparisons
```{r series_record_6}
  ...
  # Comp. 6.16:  Samples with SARS-CoV-2 infected and mock treated choroid plexus organoids cells 72 hours post infection (hpi)####
  ...
  # Comp. 6.17:  Samples with SARS-CoV-2 infected organoids cells 24hpi and mock treated choroid plexus organoids cells 72 hpi####
...
### Series record 7: Fetch study GSE152075 from GEO with 1 comparison 
```{r series_record_7}
  ...
  # Comp. 7.18 Nasopharyngeal swabs from 430 individuals with SARS-CoV-2 and 54 negative controls ####
...
### Series record 8: Fetch study GSE150392 from GEO with 1 comparison
```{r series_record_8}
 # Series 8: Fetch study GSE150392 from GEO with 1 comparison ####
  ...
  # Comp. 8.19:  Samples with SARS-CoV-2 infected and mock treated hiPSC-CMs ####
...
## Step 2: Unify to hgnc_symbols
```{r unify_gene_symbols}
...
# Sort dataframes on rownames to ensure easier comparison later on
j <- 1
ids <- ids[order(ids)]
while (j <= length(comparisons)) {
  comparisons[[j]] <- comparisons[[j]][order(row.names(comparisons[[j]])), ]
  j <- j + 1
}
```

## Step 3: DE-Analysis with DESeq2
### Run DESeq2
```{r de_analysis, echo=FALSE}
dds_list <- list()
j <- 1
while (j <= length(comparisons)) {
    # The first half are always control samples and the second halg are infected samples
    coldata <- data.frame(condition = factor(c(rep("Mock", ncol(comparisons[[j]]) / 2),
                                               rep("SARS.CoV.2", ncol(comparisons[[j]]) / 2))))
...
### Cutoffs, Volcano and DE genes
```{r cutoff_assesment}
dds_list <- readRDS("use_case_data/geo_data/data/DESeq2_result_list.rds")
volcanos <- list()
# List with the differentially expressed genes of every comparison
degs_deseq_list <- list()

# List with the Log2Fc values of every comparison. Used later to create heatmaps for the acquired pathways 
log_2_fc_list <- tibble(hgnc_symbol = ids)

# TODO Change for different cutoffs variations
fc_cutoff <- 1
pCutoff <- 0.001
...
#  Plot volcano plot to asses good cutoffs
  ...
  #)
    # Save log_2_fc_values of every comparison 
    log_2_fc_list[names(dds_list)[j]] <- results$log2FoldChange

  j <- j + 1
}
# Save log_2_fc_list ####
saveRDS(log_2_fc_list, "use_case_data/geo_data/data/log_2_fc_tibble.rds")

# Save grid plots of volcanos ####
cutoff_combination <-  paste0("LFC", gsub(pattern = "\\.",x = fc_cutoff, replacement = ","),
                              "_P", gsub(pattern = "\\.",x = pCutoff, replacement = ","))

names(volcanos) <- names(dds_list)
names(degs_deseq_list) <- names(dds_list)
for (i in c(1:length(volcanos))) {
  ggsave(plot = volcanos[[i]], filename = paste0("use_case_data/geo_data/volcano_plots/", cutoff_combination, "/",
                                                  names(volcanos)[i], ".png"), width = 9.5, height = 7)
}
# First 9 volcanos
grid <- ggarrange(plotlist = volcanos[1:9], ncol = 3, nrow = 3)
ggsave(plot = grid,
       filename = paste0("use_case_data/geo_data/volcano_plots/", cutoff_combination, "/grid_1_9.png"), 
       width = 30, height = 30)

# Last 10 volcanos
grid <- ggarrange(plotlist = volcanos[10:19], ncol = 3, nrow = 4)
ggsave(plot = grid,
       filename = paste0("use_case_data/geo_data/volcano_plots/", cutoff_combination, "/grid_10_19.png"),
       width = 30, height = 30)
```

## Step 4: Crate indicator matrix
```{r indicator_matrix}
...
upset_plot <- upset(indicator_matrix[,c(2:20)],
  ...
  sets.bar.color = "blue", nintersects = 100,
  order.by = "freq")
# TODO
pdf(file="~/Desktop/plots/geo/LFC1_P0,001/volcanos_and_upset/upset.pdf", width = 10, height = 7) # or other device
upset_plot
dev.off()
```

## Step 5: Prepare biological interaction network
```{r biological_network}
