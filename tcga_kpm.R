...
for (l in c(150, 200, 250)) {
  for (k in c(2:20)) {
    ...
    innes_results_prostate_tumor_z_3 <- kpm(graph = human_string_network, indicator_matrices = counts_matrix_z_3)
    saveRDS(innes_results_prostate_tumor_z_3, paste("use_case_data/gdc_data/kpm_results/z_score_3/prostate_tumor_z_score_3_innes_K=", k, "_L=",l,".rds", sep = ""))
  }
}
